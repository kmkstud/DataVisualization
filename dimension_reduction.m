...
M = csvimport('toydata.csv');
Data = cell2mat(M(2:end,2:end));
...
%%PCA
coeff = pca(Data(:,2:3));
[COEFF,SCORE] = princomp(Data(:,2:3));
figure;
PC1=[zeros(1000,1)-10 SCORE(:,1)];
scatter(PC1(:,2), PC1(:,1), 100, Data(:,1),'+');
title('Toydata (PCA)');
xlabel('PC 1');
...
%%Non-scalic MDS
dissimilarities = pdist(zscore(Data(:,2:3)),'cityblock');
[Y,stress,disparities] = mdscale(dissimilarities,2,'criterion','stress');
figure;
scatter(Y(:,2), PC1(:,1), 100, Data(:,1),'+');
hold on;
histogram(Y(:,2));
title('Toydata (Non metric MDS)');
xlabel('X');
ylabel('Number of samples');
figure;
...
mappedX = lle(Data(:,2:3), 1, 10);
scatter(mappedX(:,1), PC1(:,1)-10, 100, Data(:,1),'+');
hold on;
histogram(mappedX(:,1));
title('Toydata (LLE)');
xlabel('X');
